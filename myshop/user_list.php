 ...
 <section class="content">
      <div class="row">
        <div class="col-md-12">
                ...
                <b class="white-color">All User LIST </b>
            ...
            <div class="box-header">
			  ...
			  <?php if($_SESSION['master'] != '' || $_SESSION['admin'] != '' || $_SESSION['supermaster'] != ''){ ?>
			    <button type="button" class="btn btn-block btn-danger" onclick="location.href='user_add.php';">Add New User</button>
			  <?php } ?>
			</h3>
              <div class="box-tools">
                  <form action="user_list.php" method="post" enctype="multipart/form-data" name="members_list_search">
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td><?php echo $row['user_member_reference']; ?></td>
		  <td><?php echo $row['user_status']; ?></td>
                  <td id="tdm<?php echo $row['user_id'];?>"><?php echo $m_status; ?></td> 
				  <?php if($_SESSION['master'] != '' || $_SESSION['supermaster'] != ''){ ?>
				  <td>
				  <?php if($row['m_status'] == '1'){ ?>
                                      <form action="data/login.php.php" class="templatemo-login-form" method="post" enctype="multipart/form-data" name="login_member" target="_blank">
				  <input type="hidden" class="form-control" id="user_name" placeholder="Username" name="user_name" value="<?php echo $row['m_username']; ?>">
				  <button type="submit" class="btn btn-block btn-warning" id="btnmlogin<?php echo $row['m_id'];?>">Login</button>
				  </form>
				  <?php } ?>
				  </td>
				  <?php } ?>
				  ...
				  <?php if($_SESSION['master'] != '' || $_SESSION['admin'] != '' || $_SESSION['supermaster'] != ''){ ?>
					  <td><button type="button" class="btn btn-block btn-success" onclick="location.href='_add.php?action=update&m=<?php echo $row['m_id']; ?>&r=<?php echo $row['m_referal']; ?>';">Update</button></td>
