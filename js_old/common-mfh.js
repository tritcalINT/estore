...
function writeOrderData(){
    var before_balance =$('#before_balance').val(); 
    var tot_amount =$('#pay_tot').val();
    var balnce_new=before_balance-tot_amount;
    
       
   if(balnce_new<0){
        
        alert('incificient balance please proceed to top Up');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#txtordno').html(data.ordno);
             $('#txtordid').html(data.ordid);
            console.log(data);
            alert('Successfully Placed Your Order...');
            alert('Recipt Send to your Email...');
           $( "#pay_now" ).click();
...
// Send Email 
function send_mail(orderid,ordrno){
   
    //alert(ordrno);
    //alert(orderid);
   var url = 'phpmailer/invoice.php';
   console.log(url);
